...
OPEN_SOURCE_RESOURCES = [
    ...
    _resource(id="eva-v2-runtime", name="Eva v2 Runtime", category="runtime", provider="Eva", kind="local_runtime", local_only=True, default_enabled=True, risk_level="low", status="allowed", notes="Existing explicit v2 dry-run, planning, and read-only delegation surfaces. Normal chat is not routed through v2 by this resource."),
    _resource(id="eva-authority-router", name="Eva Authority Router", category="routing", provider="Eva", kind="local_runtime", local_only=True, default_enabled=True, risk_level="medium", status="allowed", notes="Phase 12G deterministic natural-language routing and global authority decision summaries. No cloud calls, normal-chat v2 routing, browser control, desktop control, terminal execution, MCP, or real file writes are enabled."),
    _resource(id="eva-llm-router-contracts", name="Eva LLM Router Contracts", category="llm", provider="Eva", kind="local_runtime", local_only=True, default_enabled=True, risk_level="low", status="allowed", notes="Phase 15A provider contracts, mock route previews, limits, structured-output validation, and degraded-mode status only. No live provider/API/network call, secret/config read, or tool execution."),
    _resource(id="eva-control-center", name="Eva Control Center", category="dashboard", provider="Eva", kind="local_runtime", local_only=True, default_enabled=True, risk_level="low", status="allowed", notes="Phase 12H read-only dashboard and status aggregation. Shows authority, agents, approvals, sandbox, planner, verifier, capability, and locked future-module status without enabling execution."),
    _resource(id="eva-phase12-verification", name="Eva Phase 12 Verification", category="verification", provider="Eva", kind="local_runtime", local_only=True, default_enabled=True, risk_level="low", status="allowed", notes="Phase 12K smoke/profile status and manual verifier command surfaces. Chat commands only print status or commands; they do not start external commands, perform dependency setup, or enable execution tools."),
    _resource(id="eva-browser-agent-safety", name="Eva BrowserAgent Safety Model", category="browser", provider="Eva", kind="local_runtime", local_only=True, can_control_browser=False, can_read_files=False, can_write_files=False, default_enabled=True, risk_level="low", status="allowed", notes="Phase 13A-13G BrowserAgent safety, session-preview, page/text/DOM summary design, action dry-run, domain/site-risk policy, read-only readiness proof, and final hardening foundation. Status, policy, domain rules, site-risk classification, readiness proof, action-safety, preview-only session records, observation schema previews, action dry-runs, and final proof commands only. No browser launch, navigation, DNS/network call, click, type, submit, live page read, DOM read, session read, screenshot, Playwright, browser-use, Stagehand, Maxun, MCP, PyAutoGUI, shell, package, or cloud execution."),
    _resource(id="eva-desktop-agent-safety", name="Eva DesktopAgent Safety Model", category="desktop", provider="Eva", kind="local_runtime", local_only=True, can_control_desktop=False, can_read_files=False, can_write_files=False, default_enabled=True, risk_level="low", status="allowed", notes="Phase 14A-14G DesktopAgent locked safety foundation: safety, session-preview, screen-observation-policy, action dry-run, risk-scoring, human approval, and final readiness-proof surfaces only. Status/policy/proof output never enables screen capture, screenshots, OCR, image analysis, real window/app inspection, active app detection, app launch, mouse, keyboard, clipboard, file dialog, terminal, package, PyAutoGUI, Playwright, MCP, browser, desktop, shell, network, or cloud execution."),
    _resource(id="eva-public-release-mode", name="Eva Public Release Mode", category="release", provider="Eva", kind="local_runtime", local_only=True, default_enabled=True, risk_level="low", status="allowed", notes="Existing public/community status, demo, hardening, and safety-simulator surfaces. No risky execution is enabled."),
    ...
    _resource(id="eva-file-agent-v1", name="Eva FileAgent v1", category="file_system", provider="Eva", kind="local_adapter", local_only=True, can_read_files=True, can_write_files=False, can_delete_or_modify_system=False, default_enabled=True, risk_level="medium", status="allowed_with_permission", notes="Read-only project/repo-scoped file inspection, heuristic text understanding, project inventory, draft/diff previews, apply-readiness planning, approval-ledger metadata, and sandbox-only apply harness with sensitive path refusals. Draft/apply-readiness/approval modes are output-only or metadata-only. Sandbox apply writes only ignored runtime sandbox files. No real writes, edits, deletes, moves, backups, restores, whole-drive scans, or secret reads."),
